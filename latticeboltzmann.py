...
u_ambient = [0, 0.05] # velocity BUG: why is anything moving when velocity is [0, 0]? (or is it just near 0 and numerically unstable)
def isBlocked(y, x):
  #return x == -10000000
  #return x == y
...
def loop(cells):
  ...
  np.nan_to_num(u, copy=False) # Is this a bad hack? if p == 0 (i.e. blocked) then we want u to be zero.
                               # BUG: actually, the default state should be the one where each outward arrow is in thermal equilibrium
  #print(u_ambient, u)
